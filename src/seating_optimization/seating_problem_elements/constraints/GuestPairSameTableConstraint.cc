...
/// @file src/seating_optimization/seating_problem_elements/constraints/GuestPairSameTableConstraint.cc
/// @brief Implementations for a GuestPairSameTableConstraint.
/// @details The GuestPairSameTableConstraint class is used to indicate that two guests should be seated at the same table,
/// or should NOT be seated at the same table.
...
GuestPairSameTableConstraint::GuestPairSameTableConstraint( GuestPairSameTableConstraint const & src ) :
	Parent( src )
{
	...
	GuestPairSameTableConstraint::protected_assign( src );
}

/// @brief Make a copy of this object.
seating_optimization_masala_plugins::seating_optimization::seating_problem_elements::SeatingElementBaseSP
GuestPairSameTableConstraint::clone() const {
	return masala::make_shared< GuestPairSameTableConstraint >( *this );
}

/// @brief Make a fully independent copy of this object.
GuestPairSameTableConstraintSP
GuestPairSameTableConstraint::deep_clone() const {
	GuestPairSameTableConstraintSP new_guest( std::static_pointer_cast< GuestPairSameTableConstraint >( this->clone() ) );
...
GuestPairSameTableConstraint::get_categories() const {
...
GuestPairSameTableConstraint::get_keywords() const {
...
/// @returns "GuestPairSameTableConstraint".
std::string
GuestPairSameTableConstraint::class_name() const {
	return "GuestPairSameTableConstraint";
}

/// @brief Get the namespace for this class.
/// @returns "seating_optimization_masala_plugins::seating_optimization::seating_problem_elements::constraints".
std::string
GuestPairSameTableConstraint::class_namespace() const {
...
/// @brief Get a description of the API for the GuestPairSameTableConstraint class.
masala::base::api::MasalaObjectAPIDefinitionCWP
GuestPairSameTableConstraint::get_api_definition() {
	...
	if( api_definition() == nullptr ) {
		...
		MasalaObjectAPIDefinitionSP api_def(
			masala::make_shared< MasalaObjectAPIDefinition >(
				*this,
				"The GuestPairSameTableConstraint class is used to indicate that two guests should be seated at the same table, "
				"or should NOT be seated at the same table.",
				false, false
			)
		);
		ADD_PUBLIC_CONSTRUCTOR_DEFINITIONS( GuestPairSameTableConstraint, api_def );
		...
		api_def->add_setter(
			masala::make_shared< MasalaObjectAPISetterDefinition_OneInput< std::string const & > >(
				...
				"a line of the form 'GuestPairSameTableConstraint <guest1_uid> <guest2_uid> <constraint_strength>'.",
				...
				false, true,
				std::bind( &GuestPairSameTableConstraint::configure_from_input_line, this, std::placeholders::_1 )
			)
		);
		...
		api_def->add_work_function(
			...
			>(
				...
				std::bind(
					&GuestPairSameTableConstraint::add_constraint_to_cfn_problem, this,
...
/// a line of the form "GuestPairSameTableConstraint <guest1_uid> <guest2_uid> <constraint_strength>".
void
GuestPairSameTableConstraint::configure_from_input_line(
	...
	CHECK_OR_THROW_FOR_CLASS( linestart == "GuestPairSameTableConstraint", "configure_from_input_line", "Expected line to begin with class name (\"GuestPairSameTableConstraint\")." );
}
...
void
GuestPairSameTableConstraint::add_constraint_to_cfn_problem(
seating_optimization_masala_plugins::seating_optimization::seating_problem::SeatingProblem const & seating_problem,
	...
	std::pair< Size, Size > const guestpair( guest1_index, guest2_index );
	Real const penalty_value( global_strength_multiplier * constraint_strength_ );
	Size const ntables( seating_problem.n_tables() );

	for( Size itable(0); itable < ntables; ++itable ) {
		std::vector< Size > const seats_at_table( seating_problem.seats_at_table( itable ) );
		for( Size const iseat : seats_at_table ) {
			if( !guest_to_allowed_seats[guest1_index][iseat] ) { continue; }
			Size const guest1_choice_index(guest_to_seat_index_to_guest_choice[guest1_index].at(iseat));
			for( Size const jseat : seats_at_table ) {
				if( !guest_to_allowed_seats[guest2_index][jseat] ) { continue; }
				if( iseat != jseat ) {
					Size const guest2_choice_index( guest_to_seat_index_to_guest_choice[guest2_index].at(jseat) );
					cfn_problem_cast->add_to_twobody_penalty( guestpair, std::make_pair( guest1_choice_index, guest2_choice_index ), penalty_value );
					write_to_tracer( "Added same-table constraint for guests " + std::to_string(guest1_index) + " and " + std::to_string(guest2_index)
						+ ", at seats " + std::to_string(iseat) + " and " + std::to_string(jseat) + ".  Penalty: "
						+ std::to_string(penalty_value) + "."
					);
				}
			}
		}
...
void
GuestPairSameTableConstraint::protected_make_independent() {
	// TODO DEEP CLONING
...
/// the parent class implementation.
void
GuestPairSameTableConstraint::protected_assign( SeatingElementBase const & src ) {
	GuestPairSameTableConstraint const * const src_ptr_cast( dynamic_cast< GuestPairSameTableConstraint const * >( &src ) );
	CHECK_OR_THROW_FOR_CLASS( src_ptr_cast != nullptr, "protected_assign", "Cannot assign an object of type " + src.class_name()
		+ " to a GuestPairSameTableConstraint object."
	);
